...
{
    [Activity(Label = "E_ScanBCD")]
    public class E_ScanBCD : Activity, View.IOnTouchListener
    {
        TextView lblHeader2;
        EditText txtBCD;
        Button btnBack;
        ...
        public void validateItem()
        {
            try
            {
                string validated = Client.ValidatePPItem(itemCode + "|" +lotNumber);
                if (validated != string.Empty)
                {
                    switch (validated.Split('*')[0])
                    {
                        case "1":
                            ...
                            GlobalVar.PPItemDesc = validated;
                            GlobalVar.PPItemCode = itemCode;
                            GlobalVar.PPLotNum = lotNumber;
        ...
        {
            StartActivity(typeof(E_ScanWeight));
            Finish();
        }
