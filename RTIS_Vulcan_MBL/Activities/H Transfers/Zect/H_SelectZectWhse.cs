...
{
    [Activity(Label = "H_SelectZectWhse")]
    public class H_SelectZectWhse : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        ...
        public void layoutStichUp()
        {
            ...
            {
                ErrorText = "Exception H701: " + System.Environment.NewLine + ex.ToString();
        ...
        public void GetWHTMSDetails()
        {
            try
            {
                string Whses = Client.getZectWarehouses(GlobalVar.ZectWhse);
                switch (Whses.Split('*')[0])
                {
                    case "1":
                        Whses = Whses.Remove(0, 2);
                        GlobalVar.ZectTransferWarehouses = new System.Collections.Generic.List<GlobalVar.ZectWarehouseDetails>();
                        string[] allWhses = Whses.Split('~');
                        foreach (string whse in allWhses)
                        {
                            if (whse != string.Empty)
                            {
                                GlobalVar.ZectWarehouseDetails whDetail = new GlobalVar.ZectWarehouseDetails();
                                whDetail.Code = whse.Split('|')[0];
                                whDetail.Description = whse.Split('|')[1];
                                GlobalVar.ZectTransferWarehouses.Add(whDetail);
                            }
                        }
                        RunOnUiThread(SetWHSpinner);
                        break;
                    case "0":
                        ErrorText = "Exception H702: " + System.Environment.NewLine + Whses.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception H703: " + System.Environment.NewLine + Whses.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
                        ErrorText = "Exception H704: " + System.Environment.NewLine + "Unexpected error while retreiving data";
                        RunOnUiThread(ShowError);
                        break;
                }
            }
            ...
            {
                ErrorText = "Exception H705: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void SetWHSpinner()
        {
            try
            {
                ...
                foreach (GlobalVar.ZectWarehouseDetails w in GlobalVar.ZectTransferWarehouses)
                {
            ...
            {
                ErrorText = "Exception H706: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void SpnWhses_ItemSelected(object sender, AdapterView.ItemSelectedEventArgs e)
        {
            try
            {
                ...
                GlobalVar.ZTWhseTo = GlobalVar.ZectTransferWarehouses[e.Position].Code;
            }
            catch (Exception ex)
            {
                ErrorText = "Exception H707: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void BtnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception H708: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void BtnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Confirm.SetMessage(GlobalVar.ZTDescription + System.Environment.NewLine +  "Lot: " + GlobalVar.ZTLotNum + System.Environment.NewLine + "Qty: " + GlobalVar.ZTQty + System.Environment.NewLine + "Warehouse To: " + GlobalVar.ZTWhseTo);
                Confirm.SetButton("YES", (s, ev) =>
                {
                    ...
                    GlobalVar.pgsBusy.SetCancelable(false);
                    GlobalVar.pgsBusy.Show();
                    Thread t = new Thread(transferZect);
                    t.Start();
                });
            ...
            {
                ErrorText = "Exception H709: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void transferZect()
        {
            try
            {
                string transferred = Client.transferZectItem(GlobalVar.ZTItemCode + "|" + GlobalVar.ZTLotNum + "|" + GlobalVar.ZTQty + "|" + GlobalVar.ZectWhse + "|" + GlobalVar.ZTWhseTo + "|" + GlobalVar.UserName + "|" + GlobalVar.ZTUnq);
