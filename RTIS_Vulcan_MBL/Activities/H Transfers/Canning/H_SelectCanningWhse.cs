...
namespace RTIS_Vulcan_MBL.Activities.H_Transfers
{
    [Activity(Label = "H_SelectCanningWhse")]
    public class H_SelectCanningWhse : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader1.Text = "Canning Transfer";
            ...
            {
                ErrorText = "Exception H901: " + System.Environment.NewLine + ex.ToString();
        ...
        public void GetWHTMSDetails()
        {
            try
            {
                string Whses = Client.getCanningWarehouses();
                switch (Whses.Split('*')[0])
                {
                    case "1":
                        Whses = Whses.Remove(0, 2);
                        GlobalVar.CanningTransferWarehouses = new System.Collections.Generic.List<GlobalVar.CanningWarehouseDetails>();
                        string[] allWhses = Whses.Split('~');
                        foreach (string whse in allWhses)
                        {
                            if (whse != string.Empty)
                            {
                                GlobalVar.CanningWarehouseDetails whDetail = new GlobalVar.CanningWarehouseDetails();
                                whDetail.Code = whse.Split('|')[0];
                                whDetail.Description = whse.Split('|')[1];
                                GlobalVar.CanningTransferWarehouses.Add(whDetail);
                            }
                        }
                        RunOnUiThread(SetWHSpinner);
                        break;
                    case "0":
                        ErrorText = "Exception H902: " + System.Environment.NewLine + Whses.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception H903: " + System.Environment.NewLine + Whses.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
                        ErrorText = "Exception H904: " + System.Environment.NewLine + "Unexpected error while retreiving data";
                        RunOnUiThread(ShowError);
                        break;
                }
            }
            ...
            {
                ErrorText = "Exception H905: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void SetWHSpinner()
        {
            try
            {
                ...
                foreach (GlobalVar.CanningWarehouseDetails w in GlobalVar.CanningTransferWarehouses)
                {
            ...
            {
                ErrorText = "Exception H906: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void SpnWhses_ItemSelected(object sender, AdapterView.ItemSelectedEventArgs e)
        {
            try
            {
                ...
                GlobalVar.CanningWhseTo = GlobalVar.CanningTransferWarehouses[e.Position].Code;
            }
            catch (Exception ex)
            {
                ErrorText = "Exception H907: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        ...
        private void BtnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Confirm.SetTitle("Confirm A&W Info");
                Confirm.SetIcon(Resource.Drawable.Icon);
                Confirm.SetMessage("Item Code: " + GlobalVar.CanningItemCode + System.Environment.NewLine + "Lot: " + GlobalVar.CanningLotNum + System.Environment.NewLine + "Qty: " + GlobalVar.CanningQty + System.Environment.NewLine + "Warehouse To: " + GlobalVar.CanningWhseTo);
                Confirm.SetButton("YES", (s, ev) =>
                {
                    ...
                    GlobalVar.pgsBusy.SetCancelable(false);
                    GlobalVar.pgsBusy.Show();
                    Thread t = new Thread(transferCanning);
                    t.Start();
                });
            ...
            {
                ErrorText = "Exception H909: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void transferCanning()
        {
            try
            {
                string transferred = Client.transferCaningItem(GlobalVar.CanningItemCode + "|" + GlobalVar.CanningLotNum + "|" + GlobalVar.CanningQty + "|" + GlobalVar.CanWhse + "|" + GlobalVar.CanningWhseTo + "|" + GlobalVar.UserName + "|" + GlobalVar.CanningUnq);
                switch (transferred.Split('*')[0])
                {
                    case "1":
                        ...
                        break;
                    case "0":
                        ErrorText = "Exception H910: " + System.Environment.NewLine + transferred.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception H911: " + System.Environment.NewLine + transferred.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
                        ErrorText = "Exception H912: " + System.Environment.NewLine + "Unexpected error while storing data";
                        RunOnUiThread(ShowError);
                        break;
                }
            }
            ...
            {
                ErrorText = "Exception H913: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
