...
{
    [Activity(Label = "H_Scan_RT2D_Canning")]
    public class H_Scan_RT2D_Canning : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtItem;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader2.Text = "Scan FG Barcode";
        ...
        private void TxtItem_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        GlobalVar.CanningItemCode = Barcodes.GetItemCode(txtItem.Text);
                        GlobalVar.CanningLotNum = Barcodes.GetItemLot(txtItem.Text);
                        GlobalVar.CanningQty = Barcodes.GetItemQty(txtItem.Text);
                        GlobalVar.CanningUnq = Barcodes.GetUniqCode(txtItem.Text);
                        //GlobalVar.AWLineID = Barcodes.GetLineID(txtItem.Text);
                        StartActivity(typeof(H_SelectCanningWhse));
                        Finish();
                    }
                    else
                    {
                        ErrorText = "Please scan an A&W barcode!";
                        RunOnUiThread(ShowError);
                    }
                }
                else
