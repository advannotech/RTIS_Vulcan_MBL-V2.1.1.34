...
{
    [Activity(Label = "H_Select_AWWhse")]
    public class H_Select_AWWhse : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        ...
        public void GetWHTMSDetails()
        {
            try
            {
                string Whses = Client.getAWWarehouses();
                switch (Whses.Split('*')[0])
                {
                    case "1":
                        Whses = Whses.Remove(0, 2);
                        GlobalVar.AWTransferWarehouses = new System.Collections.Generic.List<GlobalVar.AWWarehouseDetails>();
                        string[] allWhses = Whses.Split('~');
                        foreach (string whse in allWhses)
                        {
                            if (whse != string.Empty)
                            {
                                GlobalVar.AWWarehouseDetails whDetail = new GlobalVar.AWWarehouseDetails();
                                whDetail.Code = whse.Split('|')[0];
                                whDetail.Description = whse.Split('|')[1];
                                GlobalVar.AWTransferWarehouses.Add(whDetail);
                            }
                        }
        ...
        public void SetWHSpinner()
        {
            try
            {
                ...
                foreach (GlobalVar.AWWarehouseDetails w in GlobalVar.AWTransferWarehouses)
                {
        ...
        private void SpnWhses_ItemSelected(object sender, AdapterView.ItemSelectedEventArgs e)
        {
            try
            {
                ...
                GlobalVar.AWWhseTo = GlobalVar.AWTransferWarehouses[e.Position].Code;
            }
            catch (Exception ex)
            {
        ...
        private void BtnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Confirm.SetMessage("Item Code: " + GlobalVar.AWItemCode + System.Environment.NewLine + "Lot: " + GlobalVar.AWLotNum + System.Environment.NewLine + "Qty: " + GlobalVar.AWQty + System.Environment.NewLine + "Warehouse To: " + GlobalVar.AWWhseTo);
                Confirm.SetButton("YES", (s, ev) =>
                {
                    ...
                    GlobalVar.pgsBusy.SetCancelable(false);
                    GlobalVar.pgsBusy.Show();
                    Thread t = new Thread(transferAW);
                    t.Start();
                });
        ...
        public void transferAW()
        {
            try
            {
                string transferred = Client.transferAWItem(GlobalVar.AWItemCode + "|" + GlobalVar.AWLotNum + "|" + GlobalVar.AWQty + "|" + GlobalVar.AWWhse + "|" + GlobalVar.AWWhseTo + "|" + GlobalVar.UserName + "|" + GlobalVar.AWUnq);
