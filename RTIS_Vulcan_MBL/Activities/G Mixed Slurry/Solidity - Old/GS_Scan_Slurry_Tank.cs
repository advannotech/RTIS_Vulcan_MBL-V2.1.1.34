...
{
    [Activity(Label = "GS_Scan_Slurry_Tank")]
    public class GS_Scan_Slurry_Tank : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader1.Text = "Mixed Slurry Solidity";
            ...
            {
                ErrorText = "Exception GS001: " + System.Environment.NewLine + ex.ToString();
        ...
        private void TxtTank_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        if (txtTank.Text.Substring(0, 4) == "TNK_" && txtTank.Text.Contains("$"))
                        {
                            string barcode = txtTank.Text;
                            barcode = barcode.Substring(4, barcode.Length - 4);
                            tankNo = barcode.Split('$')[0]; 
            ...
            {
                ErrorText = "Exception GS002: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        ...
        public void getTankInfo()
        {
            try
            {
                string tankInfo = Client.getTankInfoForSolidity(tankNo + "|" + itemCode);
                switch (tankInfo.Split('*')[0])
                {
                    case "1":
                        tankInfo = tankInfo.Remove(0, 2);
                        GlobalVar.MSSTankCode = tankNo;
                        GlobalVar.MSSItemCode = itemCode;
                        GlobalVar.MSSCurrentWeight = tankInfo.Split('|')[0];
                        GlobalVar.MSSLotNum = tankInfo.Split('|')[1];
                        GlobalVar.MSSItemDesc = tankInfo.Split('|')[2];
                        RunOnUiThread(showNext);
                        break;
                    case "0":
                        ErrorText = "Exception GS003: " + System.Environment.NewLine + tankInfo.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception GS004: " + System.Environment.NewLine + tankInfo.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
                        ErrorText = "Exception GS005: " + System.Environment.NewLine + "Unexpected error while storing data";
                        RunOnUiThread(ShowError);
                        break;
                }
            }
            ...
            {
                ErrorText = "Exception GS006: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        ...
        public void showNext()
        {
            try
            {
                StartActivity(typeof(GS_Enter_Solidity));
                Finish();
            }
            catch (Exception ex)
            {
                ErrorText = "Exception GS007: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        ...
        public void reset()
        {
            ...
            {
                ErrorText = "Exception GS008: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        ...
        private void BtnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception GS009: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
