...
namespace RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry.Solidity
{
    [Activity(Label = "G_EnterSolidity_Sol")]
    public class G_EnterSolidity_Sol : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        ...
        public void showConfirm()
        {
            try
            {
                ...
                Confirm.SetMessage("Tank Number: " + GlobalVar.MSTankCode + System.Environment.NewLine + "Item: " + GlobalVar.MSDescSol + System.Environment.NewLine + "Lot: " + GlobalVar.MSLotSol + System.Environment.NewLine + "Wet Weight: " + GlobalVar.MSWeightSol + System.Environment.NewLine + "Solidity: " + GlobalVar.MSSolidity);
                Confirm.SetButton("YES", (s, ev) =>
                {
                    ...
                    GlobalVar.pgsBusy.SetMessage("Setting Solidity...");
                    GlobalVar.pgsBusy.SetCancelable(false);
                    GlobalVar.pgsBusy.Show();
                    Thread t = new Thread(addSolidity);
                    t.Start();
                });
                Confirm.SetButton2("NO", (s, ev) =>
                {
                    ScanEngineOn();
                    StartActivity(typeof(G_EnterSolidity_Sol));
                    Finish();
                });
                Confirm.Show();
            }
        ...
        public void addSolidity()
        {
            try
            {
                string soliditySaved = Client.setMixedSlurrySolidity(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode + "|" + GlobalVar.MSWeightSol + "|" + GlobalVar.MSSolidity + "|" + GlobalVar.UserName);
                if (soliditySaved != string.Empty)
                {
                    switch (soliditySaved.Split('*')[0])
                    {
                        case "1":
                            ...
                            break;
                        case "0":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + soliditySaved.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + soliditySaved.Split('*')[1];
                            ...
                            break;
                        default:
                            ErrorText = "Exception G104: Unexpected result from server!" + System.Environment.NewLine + soliditySaved;
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
        ...
        private void BtnBack_Click(object sender, EventArgs e)
        {
            try
            {
                StartActivity(typeof(G_EnterWetWeight_Sol));
                Finish();
            }
            catch (Exception ex)
            {
