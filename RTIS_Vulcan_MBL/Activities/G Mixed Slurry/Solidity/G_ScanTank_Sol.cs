...
{
    [Activity(Label = "G_ScanTank_Sol")]
    public class G_ScanTank_Sol : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
                ...
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtTank.Text != string.Empty)
                    {
                        if ((txtTank.Text.Substring(0, 5) == "MTNK_" || txtTank.Text.Substring(0, 4) == "TNK_") && txtTank.Text.Contains("$"))
                        {
                            ...
                            GlobalVar.pgsBusy.SetMessage("Checking Tank...");
                        ...
                        {
                            ErrorText = "Please scan a valid tank to decant!";
                            RunOnUiThread(ShowError);
                        }
                    }
                    else
                    {
                        ErrorText = "Please a the tank barcode!";
                        RunOnUiThread(ShowError);
                    }
                }
                else
        ...
        public void checkTank()
        {
            try
            {
                string tankChecked = Client.getTankInfoSolidity(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode);
                try
                {
                ...
                if (tankChecked != string.Empty)
                {
                    ...
                    {
                        case "1":
                            tankChecked = tankChecked.Remove(0, 2);
                            GlobalVar.MSDescSol = tankChecked.Split('|')[1];
                            GlobalVar.MSLotSol = tankChecked.Split('|')[0];
