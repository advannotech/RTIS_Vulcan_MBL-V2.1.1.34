...
{
    [Activity(Label = "G_ScanTrolley_FS")]
    public class G_ScanTrolley_FS : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        private void TxtTank_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        if (txtTank.Text.Substring(0, 4) == "TRO_" && txtTank.Text.Contains("$"))
                        {
                            string barcode = txtTank.Text;
                            barcode = barcode.Split('_')[1];
                            GlobalVar.MSTrolleyCode = barcode.Split('$')[0];
                            GlobalVar.MSSlurryCode = barcode.Split('$')[1];
                            ...
                            Thread t = new Thread(getTrolleyInfo);
                            t.Start();
                        }
                        else
                        {
        ...
        public void getTrolleyInfo()
        {
            try
            {
                string trolleyChecked = Client.checkSlurryTrolley(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode + "|" + GlobalVar.MSTrolleyCode + "|" + GlobalVar.MSSlurryCode);
                try
                {
                ...
                if (trolleyChecked != string.Empty)
                {
                    switch (trolleyChecked.Split('*')[0])
                    {
                        case "1":
                            trolleyChecked = trolleyChecked.Remove(0, 2);
                            GlobalVar.MSSlurryLot = trolleyChecked.Split('|')[0];
                            GlobalVar.MSSlurryDesc = trolleyChecked.Split('|')[1];
                            RunOnUiThread(showNext);
                            break;
                        case "0":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + trolleyChecked.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + trolleyChecked.Split('*')[1];
                            ...
                            break;
                        default:
                            ErrorText = "Exception G104: Unexpected result from server!" + System.Environment.NewLine + trolleyChecked;
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
        ...
        private void BtnBack_Click(object sender, EventArgs e)
        {
            try
            {
                StartActivity(typeof(G_SlurryView_FS));
                Finish();
            }
            catch (Exception ex)
            {
