...
using Android.Media;
using RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry.Add_Fresh_Slurry;

namespace RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry
{
    [Activity(Label = "G_EnterSlurryWeight_FS")]
    public class G_EnterSlurryWeight_FS : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader1.Text = "Mixed Slurry Addition";
            ...
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtWeight2.Text.All(char.IsDigit))
                    {
                        ...
                        GlobalVar.MSSlurryWeight = qty;
                        AlertDialog.Builder build = new AlertDialog.Builder(this);
                        AlertDialog Confirm = build.Create();
                        Confirm.SetTitle("Slurry information");
                        Confirm.SetIcon(Resource.Drawable.Icon);
                        Confirm.SetMessage("Trolley Number: " + GlobalVar.MSTrolleyCode + System.Environment.NewLine + "Slurry: " + GlobalVar.MSSlurryDesc + System.Environment.NewLine + "Lot Number: " + GlobalVar.MSSlurryLot + System.Environment.NewLine + "Wet Weight: " + GlobalVar.MSSlurryWeight);
                        Confirm.SetButton("YES", (s, ev) =>
                        {
                            ...
                            Thread t = new Thread(insertFreshSlurryUsed);
                            t.Start();
                        });
                        Confirm.SetButton2("NO", (s, ev) =>
                        {
                            ScanEngineOn();
                            StartActivity(typeof(G_EnterSlurryWeight_FS));
                            Finish();
                        });
                        Confirm.Show();
                    }
        ...
        public void insertFreshSlurryUsed()
        {
            try
            {
                string freshSlurryAdded = Client.saveFreshSlurryAddition(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode + "|" + GlobalVar.MSItemDesc + "|" + GlobalVar.MSTrolleyCode + "|" + GlobalVar.MSSlurryCode + "|" + GlobalVar.MSSlurryDesc + "|" + GlobalVar.MSSlurryLot + "|" + GlobalVar.MSSlurryWeight + "|" + GlobalVar.UserName);
                try
                {
                ...
                if (freshSlurryAdded != string.Empty)
                {
                    switch (freshSlurryAdded.Split('*')[0])
                    {
                        case "1":
                            ...
                            break;
                        case "0":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + freshSlurryAdded.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + freshSlurryAdded.Split('*')[1];
                            ...
                            break;
                        default:
                            ErrorText = "Exception G104: Unexpected result from server!" + System.Environment.NewLine + freshSlurryAdded;
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
