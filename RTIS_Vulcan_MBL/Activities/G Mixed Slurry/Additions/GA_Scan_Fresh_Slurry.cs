...
{
    [Activity(Label = "GA_Scan_Fresh_Slurry")]
    public class GA_Scan_Fresh_Slurry : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTrolley;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                txtTrolley.KeyPress += TxtTank_KeyPress;
            ...
            {
                ErrorText = "Exception GA101: " + System.Environment.NewLine + ex.ToString();
        ...
        private void TxtTank_KeyPress(object sender, View.KeyEventArgs e)
        {
                ...
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtTrolley.Text != string.Empty)
                    {
                        if (txtTrolley.Text.Substring(0, 4) == "TRO_" && txtTrolley.Text.Contains("$"))
                        {
                            ...
                            string trolley = barcode.Split('$')[0]; //GlobalVar.MSATankCode = barcode.Split('$')[0];
                            string item = barcode.Split('$')[1]; //GlobalVar.MSAItemCode = barcode.Split('$')[1];
                            if (GlobalVar.MSAItemCode == item)
                            {
                                GlobalVar.MSAFreshTrolley = trolley;
                                StartActivity(typeof(GA_Enter_Wet_Weight));
                                Finish();
                            }
                            else
                            {
                                ErrorText = "The trolley scanned contains an invalid slurry!";
                                RunOnUiThread(ShowError);
                            }
                        }
                        else
            ...
            {
                ErrorText = "Exception GA102: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        ...
        private void BtnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception GA103: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
