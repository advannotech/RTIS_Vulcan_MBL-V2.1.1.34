...
{
    [Activity(Label = "GA_Scan_Slurry_Tank")]
    public class GA_Scan_Slurry_Tank : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        public void layoutStichUp()
        {
            ...
            {
                ErrorText = "Exception GA001: " + System.Environment.NewLine + ex.ToString();
        ...
        private void TxtTank_KeyPress(object sender, View.KeyEventArgs e)
        {
                ...
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtTank.Text != string.Empty)
                    {
                        if (txtTank.Text.Substring(0, 4) == "TNK_" && txtTank.Text.Contains("$"))
                        {
                            ...
                            tankNo = barcode.Split('$')[0]; //GlobalVar.MSATankCode = barcode.Split('$')[0];
                            itemCode = barcode.Split('$')[1]; //GlobalVar.MSAItemCode = barcode.Split('$')[1];
            ...
            {
                ErrorText = "Exception GA002: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void getTankInfo()
        {
            try
            {
                string tankInfo = Client.getTankInfo(tankNo + "|" + itemCode);
                switch (tankInfo.Split('*')[0])
                {
                    case "1":
                        tankInfo = tankInfo.Remove(0, 2);
                        GlobalVar.MSATankCode = tankNo;
                        GlobalVar.MSAItemCode = itemCode;
                        GlobalVar.MSACurrentWeight = tankInfo.Split('|')[0];
                        GlobalVar.MSALotNum = tankInfo.Split('|')[1];
                        GlobalVar.MSAItemDesc = tankInfo.Split('|')[2];

                        RunOnUiThread(showNext);
                        break;
                    case "0":
                        ErrorText = "Exception F205: " + System.Environment.NewLine + tankInfo.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception F206: " + System.Environment.NewLine + tankInfo.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
        ...
        private void BtnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception GA003: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
