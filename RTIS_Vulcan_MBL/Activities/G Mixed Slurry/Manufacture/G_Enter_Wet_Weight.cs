...
{
    [Activity(Label = "G_Enter_Wet_Weight")]
    public class G_Enter_Wet_Weight : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        ...
        public void layoutStichUp()
        {
            ...
            {
                ErrorText = "Exception G401: " + System.Environment.NewLine + ex.ToString();
        ...
        private void TxtWeight1_KeyPress(object sender, View.KeyEventArgs e)
        {
            ...
            {
                ErrorText = "Exception G402: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void TxtWeight2_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtWeight2.Text.All(char.IsDigit))
                    {
                        ...
                        Confirm.SetMessage("Tank Code: " + GlobalVar.MSTankCode + System.Environment.NewLine + "Item: " + GlobalVar.MSItemDesc + System.Environment.NewLine + "Tank Lot: " + GlobalVar.MSLotNum + System.Environment.NewLine + "Current Weight: " + Convert.ToString(GlobalVar.MSCurrentWeight) + System.Environment.NewLine + "Trolley Weight: " + Convert.ToString(qty));
                        Confirm.SetButton("YES", (s, ev) =>
                        {
                            ...
                            Thread t = new Thread(manufSlurry);
                            t.Start();
                        });
            ...
            {
                ErrorText = "Exception G403: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }       
        private void BtnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception G404: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void manufSlurry()
        {
            try
            {
                string manufactured = Client.manufactureMixedSlurry(GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode + "|" + GlobalVar.MSLotNum + "|" + GlobalVar.MSCurrentWeight + "|" + GlobalVar.MSFreshTrolley + "|" + qty + "|" + GlobalVar.UserName);
                switch (manufactured.Split('*')[0])
                {
                    case "1":
                        ...
                        break;
                    case "0":
                        ErrorText = "Exception F205: " + System.Environment.NewLine + manufactured.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception F206: " + System.Environment.NewLine + manufactured.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
                        ErrorText = "Exception F207: " + System.Environment.NewLine + "Unexpected error while storing data";
                        RunOnUiThread(ShowError);
                        break;
                }
            }
            ...
            {
                ErrorText = "Exception G405: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void BtnBack_Click(object sender, EventArgs e)
        {
            try
            {
                StartActivity(typeof(G_Scan_Fresh_Surry));
                Finish();
            }
            catch (Exception ex)
            {
        ...
        {
            StartActivity(typeof(G_Enter_Wet_Weight));
            Finish();
        }
