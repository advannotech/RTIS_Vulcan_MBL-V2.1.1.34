...
{
    [Activity(Label = "G_Scan_Slurry_Tank")]
    public class G_Scan_Slurry_Tank : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        public void getItemDesc()
        {
            try
            {
                string itemDesc = Client.getItemDescription(GlobalVar.MSItemCode);
                if (itemDesc != string.Empty)
                {
                    switch (itemDesc.Split('*')[0])
                    {
                        case "1":
                            ...
                            GlobalVar.MSItemDesc = itemDesc;
                            RunOnUiThread(showNextMS);
                            break;
                        case "0":
