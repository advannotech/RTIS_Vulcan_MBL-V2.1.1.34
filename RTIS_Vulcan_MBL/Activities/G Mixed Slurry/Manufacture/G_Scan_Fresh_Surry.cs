...
{
    [Activity(Label = "G_Scan_Fresh_Surry")]
    public class G_Scan_Fresh_Surry : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTrolley;
        ...
        public void layoutStichUp()
        {
            ...
            {
                ErrorText = "Exception G301: " + System.Environment.NewLine + ex.ToString();
        ...
        private void TxtTank_KeyPress(object sender, View.KeyEventArgs e)
        {
                ...
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtTrolley.Text != string.Empty)
                    {
                        if (txtTrolley.Text.Substring(0, 4) == "TRO_" && txtTrolley.Text.Contains("$"))
                        {
                            ...
                            GlobalVar.MSFreshTrolley = barcode.Split('$')[0];
                            string trolleyItemCode = barcode.Split('$')[1];
                            if (GlobalVar.MSItemCode == trolleyItemCode)
                            {
            ...
            {
                ErrorText = "Exception G302: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void BtnBack_Click(object sender, EventArgs e)
        {
            try
            {
                StartActivity(typeof(G_Enter_New_Lot));
                Finish();
            }
            catch (Exception ex)
            {
                ErrorText = "Exception G303: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
