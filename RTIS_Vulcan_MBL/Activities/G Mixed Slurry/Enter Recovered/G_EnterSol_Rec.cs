...
namespace RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry.Enter_Recovered
{
    [Activity(Label = "G_EnterSol_Rec")]
    public class G_EnterSol_Rec : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
            ...
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtWeight2.Text.All(char.IsDigit))
                    {
                        ...
                        GlobalVar.MSRecSol = solidity;
                        AlertDialog.Builder build = new AlertDialog.Builder(this);
                        AlertDialog Confirm = build.Create();
                        Confirm.SetTitle("Mix information");
                        Confirm.SetIcon(Resource.Drawable.Icon);
                        Confirm.SetMessage("Tank Number: " + GlobalVar.MSTankCode + System.Environment.NewLine + "Lot Number: " + GlobalVar.MSRecLot + System.Environment.NewLine + "Wet Weight: " + GlobalVar.MSRecWeight + System.Environment.NewLine + "Solidity: " + GlobalVar.MSRecSol);
                        Confirm.SetButton("YES", (s, ev) =>
                        {
                            ...
                            Thread t = new Thread(insertRecovered);
                            t.Start();
                        });
                        Confirm.SetButton2("NO", (s, ev) =>
                        {
                            ScanEngineOn();
                            StartActivity(typeof(G_EnterSol_Rec));
                            Finish();
                        });
                        Confirm.Show();
                    }
        ...
        public void insertRecovered()
        {
            try
            {
                string remainingCaptured = Client.setMixedSlurryRecovered(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode + "|" + GlobalVar.MSRecLot + "|" + GlobalVar.MSRecSol + "|" + GlobalVar.MSRecWeight + "|" + GlobalVar.UserName);
