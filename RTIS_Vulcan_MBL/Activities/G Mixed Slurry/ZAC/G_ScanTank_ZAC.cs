...
using Android.Media;
using RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry.ZAC;

namespace RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry
{
    [Activity(Label = "G_ScanTank_ZAC")]
    public class G_ScanTank_ZAC : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader1.Text = "Zonen and Charging";
                ...
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtTank.Text != string.Empty)
                    {
                        if (( txtTank.Text.Substring(0, 5) == "MTNK_" || txtTank.Text.Substring(0, 4) == "TNK_") && txtTank.Text.Contains("$"))
                        {
                            ...
                            GlobalVar.pgsBusy.SetMessage("Checking Buffer Tank...");
        ...
        public void checkTank()
        {
            try
            {
                string tankChecked = Client.checkTankValidZAC(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode);
                try
                {
                ...
                if (tankChecked != string.Empty)
                {
                    switch (tankChecked.Split('*')[0])
                    {
                        case "1":
                            ...
                            GlobalVar.MSZACDesc = tankChecked.Split('|')[1];
                            GlobalVar.MSZACLot = tankChecked.Split('|')[0];
