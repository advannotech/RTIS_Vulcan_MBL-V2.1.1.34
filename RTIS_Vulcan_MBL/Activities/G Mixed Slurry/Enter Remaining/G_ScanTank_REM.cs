...
{
    [Activity(Label = "G_ScanTank_REM")]
    public class G_ScanTank_REM : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        public void checkTank()
        {
            try
            {
                string tankChecked = Client.checkMizedSlurryRem(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode);
                try
                {
        ...
        public void showNext()
        {
            try
            {
                StartActivity(typeof(G_EnterQty_REM));
                Finish();
            }
            catch (Exception ex)
            {
