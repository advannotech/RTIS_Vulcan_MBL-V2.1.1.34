...
using Android.Media;
using RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry.Enter_Remaining;

namespace RTIS_Vulcan_MBL.Activities.G_Mixed_Slurry.Enter_Remaining
{
    [Activity(Label = "G_EnterSol_REM")]
    public class G_EnterSol_REM : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader1.Text = "Mixed Slurry Remainder";
        ...
        private void TxtWeight2_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        solidity = txtWeight1.Text + GlobalVar.sep + txtWeight2.Text;
                        GlobalVar.MSRemSol = solidity;
                        AlertDialog.Builder build = new AlertDialog.Builder(this);
                        AlertDialog Confirm = build.Create();
                        Confirm.SetTitle("Mix information");
                        Confirm.SetIcon(Resource.Drawable.Icon);
                        Confirm.SetMessage("Tank Number: " + GlobalVar.MSTankCode + System.Environment.NewLine + "Lot Number: " + GlobalVar.MSRemLot + System.Environment.NewLine + "Wet Weight: " + GlobalVar.MSRemWeight + System.Environment.NewLine + "Solidity: " + GlobalVar.MSRemSol);
                        Confirm.SetButton("YES", (s, ev) =>
                        {
                            ...
                            Thread t = new Thread(insertRemaining);
                            t.Start();
                        });
                        Confirm.SetButton2("NO", (s, ev) =>
                        {
                            ScanEngineOn();
                            StartActivity(typeof(G_EnterSol_REM));
                            Finish();
                        });
                        Confirm.Show();
                    }
        ...
        public void insertRemaining()
        {
            try
            {
                string remainingCaptured = Client.setMixedSlurryRemaining(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode + "|" + GlobalVar.MSRemLot + "|" + GlobalVar.MSRemSol + "|" + GlobalVar.MSRemWeight + "|" + GlobalVar.UserName);
                if (remainingCaptured != string.Empty)
                {
                    switch (remainingCaptured.Split('*')[0])
                    {
                        case "1":
                            ...
                            break;
                        case "0":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + remainingCaptured.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + remainingCaptured.Split('*')[1];
                            ...
                            break;
                        default:
                            ErrorText = "Exception G104: Unexpected result from server!" + System.Environment.NewLine + remainingCaptured;
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
        ...
        public void showNext()
        {
            try
            {
                StartActivity(typeof(G_EnterLot_REM));
                Finish();
            }
            catch (Exception ex)
            {
