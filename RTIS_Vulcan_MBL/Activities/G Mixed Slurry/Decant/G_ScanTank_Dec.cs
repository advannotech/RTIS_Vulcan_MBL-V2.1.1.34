...
{
    [Activity(Label = "G_ScanTank_Dec")]
    public class G_ScanTank_Dec : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        private void TxtTank_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        if (txtTank.Text.Substring(0, 5) == "BTNK_" && txtTank.Text.Contains("$"))
                        {
        ...
        public void checkTank()
        {
            try
            {
                string tankChecked = Client.checkBufferTankValid(GlobalVar.MSTankType + "|" + GlobalVar.MSTankCode + "|" + GlobalVar.MSItemCode);
                try
                {
                ...
                if (tankChecked != string.Empty)
                {
                    switch (tankChecked.Split('*')[0])
                    {
                        case "1":
                            ...
                            GlobalVar.MSDecantDesc = tankChecked.Split('|')[1];
                            GlobalVar.MSDecantLot = tankChecked.Split('|')[0];
