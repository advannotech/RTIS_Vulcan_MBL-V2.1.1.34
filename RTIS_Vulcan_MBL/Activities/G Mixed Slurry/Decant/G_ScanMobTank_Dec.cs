...
{
    [Activity(Label = "G_ScanMobTank_Dec")]
    public class G_ScanMobTank_Dec : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTank;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader1.Text = "Mixed Slurry Decant";

                lblHeader2 = FindViewById<TextView>(Resource.Id.lblHeader2);
                lblHeader2.Text = "Scan Mobile Tank";
        ...
        private void TxtTank_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        if (txtTank.Text.Substring(0, 5) == "MTNK_" && txtTank.Text.Contains("$"))
                        {
                            string barcode = txtTank.Text;
                            barcode = barcode.Split('_')[1];
                            string check = txtTank.Text.Split('_')[0];
                            GlobalVar.MSDecantTankType = txtTank.Text.Split('_')[0];
                            GlobalVar.MSDecantTankCode = barcode.Split('$')[0];
                            GlobalVar.MSDecantItemCode = barcode.Split('$')[1];
                            ...
                            GlobalVar.pgsBusy.SetMessage("Checking Mobile Tank...");
        ...
        public void checkTank()
        {
            try
            {
                string tankChecked = Client.checkMobileTankValid(GlobalVar.MSDecantTankType + "|" + GlobalVar.MSDecantTankCode + "|" + GlobalVar.MSDecantItemCode);
                try
                {
