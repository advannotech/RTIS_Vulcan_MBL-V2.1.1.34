...
{
    [Activity(Label = "Q_Scan_Pallet")]
    public class Q_Scan_Pallet : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtPallet;
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader2.Text = "Scan Pallet";

                txtPallet = FindViewById<EditText>(Resource.Id.txtTank);
                txtPallet.KeyPress += TxtPallet_KeyPress;
                //txtPallet.Text =
                //    "(240)CHEM-1410                (15)      (10)#PALLET#            (30)00000001(90)P150620084920";
            ...
            {
                ErrorText = "Exception Q101: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void TxtPallet_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (!string.IsNullOrWhiteSpace(txtPallet.Text))
                    {
                        ...
                        GlobalVar.pgsBusy.SetMessage("Getting pallet information...");
                        GlobalVar.pgsBusy.SetCancelable(false);
                        GlobalVar.pgsBusy.Show();
                        Thread t = new Thread(GetPalletInfo);
                        t.Start();
                    }
                    else
                    {
        ...
        private void GetPalletInfo()
        {
            try
            {
                string palletInfo = Client.GetPalletInfo(txtPallet.Text);
                if (palletInfo != string.Empty)
                {
                    switch (palletInfo.Split('*')[0])
                    {
                        case "1":
                            GlobalVar.PalletList = new List<string>();
                            palletInfo = palletInfo.Remove(0, 2);
                            string[] allPalletInfo = palletInfo.Split('~');
                            foreach (string pallet in allPalletInfo)
                            {
                                if (!string.IsNullOrWhiteSpace(pallet))
                                {
                                    GlobalVar.PalletList.Add(pallet);
                                }
                            }

                            GlobalVar.ScannedPallet = txtPallet.Text;
                            StartActivity(typeof(Q_ScanItem));
                            Finish();
                            break;
                        case "0":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + palletInfo.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception G104: " + System.Environment.NewLine + palletInfo.Split('*')[1];
                            ...
                            break;
                        default:
                            ErrorText = "Exception G104: Unexpected result from server!" + System.Environment.NewLine + palletInfo;
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
