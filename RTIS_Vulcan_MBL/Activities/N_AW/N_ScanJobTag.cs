...
{
    [Activity(Label = "N_ScanJobTag")]
    public class N_ScanJobTag : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtJob;
        ...
        public void layoutStichUp()
        {
            ...
            lblHeader2.Text = "Scan Job Tag";

            txtJob = FindViewById<EditText>(Resource.Id.txtTank);
            txtJob.KeyPress += TxtJob_KeyPress;
        ...
        private void TxtJob_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        if (txtJob.Text.Substring(0, 4) == "A&W_")
                        {
                            GlobalVar.AWJobNo = txtJob.Text;
                            ...
                            GlobalVar.pgsBusy.SetMessage("Getting Job Information...");
                            GlobalVar.pgsBusy.SetCancelable(false);
                            GlobalVar.pgsBusy.Show();
                            Thread t = new Thread(getAWInformation);
                            t.Start();
                        }
                        else
                        {
                            ErrorText = "Please scan a config tag barcode!";
            ...
            {
                ErrorText = "Exception N001: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void getAWInformation()
        {
            try
            {
                string awItemInfo = Client.getAWJobInfo(txtJob.Text);
                if (awItemInfo != string.Empty)
                {
                    switch (awItemInfo.Split('*')[0])
                    {
                        case "1":
                            try
                            {
                            ...
                            awItemInfo = awItemInfo.Remove(0, 2);
                            GlobalVar.AWItemCode = awItemInfo.Split('|')[0];
                            GlobalVar.AWLotNum = awItemInfo.Split('|')[1];

                            RunOnUiThread(showNext);
                            break;
                        case "0":
                            ErrorText = "Exception H711: " + System.Environment.NewLine + awItemInfo.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception H711: " + System.Environment.NewLine + awItemInfo.Split('*')[1];
