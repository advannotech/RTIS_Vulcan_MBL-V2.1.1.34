...
{
    [Activity(Label = "N_ScanRawMaterial")]
    public class N_ScanRawMaterial : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtRM;
        ...
        {
            StartActivity(typeof(N_ScanRawMaterial));
            Finish();
        }
        #endregion
        ...
        public void layoutStichUp()
        {
            ...
            lblHeader1.Text = "A&W RM Input";

            lblHeader2 = FindViewById<TextView>(Resource.Id.lblHeader2);
            lblHeader2.Text = "Scan ZECT Tag";

            txtRM = FindViewById<EditText>(Resource.Id.txtTank);
            txtRM.KeyPress += TxtRM_KeyPress;
        ...
        private void TxtRM_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        if (txtRM.Text.Contains("(") && txtRM.Text.Contains(")"))
                        {
                            GlobalVar.AWRMCode = Barcodes.GetItemCode(txtRM.Text);
                            GlobalVar.AWRMLot = Barcodes.GetItemLot(txtRM.Text);
                            GlobalVar.AWQty = Barcodes.GetItemQty(txtRM.Text);
                            GlobalVar.AWUnq = Barcodes.GetUniqCode(txtRM.Text);

                            string baseCodeRM = GlobalVar.AWRMCode.Replace("-1st COAT", string.Empty).Replace("-2nd COAT", string.Empty).Replace("-3rd COAT", string.Empty).Replace("-4rd COAT", string.Empty).Replace(" - 1st COAT", string.Empty).Replace(" - 2nd COAT", string.Empty).Replace(" - 3rd COAT", string.Empty).Replace(" - 4rd COAT", string.Empty).Replace(" ", string.Empty);
                            string baseCodeFG = GlobalVar.AWItemCode.Replace("-A&W", string.Empty);
                            if (baseCodeFG == baseCodeRM)
                            {
                                AlertDialog.Builder build = new AlertDialog.Builder(this);
                                AlertDialog Confirm = build.Create();
                                Confirm.SetTitle("Confirm Raw Material Issue");
                                Confirm.SetIcon(Resource.Drawable.Icon);
                                Confirm.SetMessage("Job Number: " + GlobalVar.AWJobNo + System.Environment.NewLine + "Item Code: " + GlobalVar.AWRMCode + System.Environment.NewLine + "Lot: " + GlobalVar.AWRMLot + System.Environment.NewLine + "Qty Weight: " + GlobalVar.AWQty + System.Environment.NewLine);
                                Confirm.SetButton("YES", (s, ev) =>
                                {
                                    ...
                                    GlobalVar.pgsBusy.SetMessage("Issuing Raw Material...");
                                    GlobalVar.pgsBusy.SetCancelable(false);
                                    GlobalVar.pgsBusy.Show();
                                    Thread t = new Thread(addAWRMInput);
                                    t.Start();
                                });
                            ...
                            {
                                ErrorText = "Invalid ZECT catalyst scanned for job!";
                                RunOnUiThread(ShowError);
                            }
                        }
                        else
                        {
                            ErrorText = "Please scan a valid ZECT tag barcode!";
                            RunOnUiThread(ShowError);
                        }
                    }
                    else
                    {
                        ErrorText = "Please scan a ZECT tag barcode!";
                        RunOnUiThread(ShowError);
                    }
                }
                else
            ...
            {
                ErrorText = "Exception N002: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void addAWRMInput()
        {
            try
            {
                string issued = Client.issueAwRM(GlobalVar.AWJobNo + "|" + GlobalVar.AWRMCode + "|" + GlobalVar.AWRMLot + "|" + GlobalVar.AWQty + "|" + GlobalVar.AWUnq +  "|" + GlobalVar.UserName);
                if (issued != string.Empty)
                {
                    switch (issued.Split('*')[0])
                    {
                        case "1":
                            SuccessText = "ZECT catalyst issued successfully!";
                            RunOnUiThread(showSuccess);
                            break;
                        case "0":
                            ErrorText = "Exception H711: " + System.Environment.NewLine + issued.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception H711: " + System.Environment.NewLine + issued.Split('*')[1];
                ...
                {
                    ErrorText = "No data was returned from the server!";
                    RunOnUiThread(ShowError);
                }
            }
            catch (Exception ex)
