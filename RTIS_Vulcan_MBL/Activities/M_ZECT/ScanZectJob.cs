...
{
    [Activity(Label = "ScanZectJob")]
    public class ScanZectJob : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtItem;
            ...
            try
            {
                switch (GlobalVar.ProcessName)
                {
                    case "ZECT1":
                        ...
                        lblHeader2.Text = "Scan Config Tag Barcode";
                        break;
                    case "ZECT2":
                        lblHeader1 = FindViewById<TextView>(Resource.Id.lblHeader1);
        ...
        private void TxtItem_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtItem.Text.Contains("ZECT_"))
                    {
                        ...
                        GlobalVar.pgsBusy.SetMessage("Checking Job...");
                        GlobalVar.pgsBusy.SetCancelable(false);
                        GlobalVar.pgsBusy.Show();
                        Thread t = new Thread(checkZectJob);
                        t.Start();
                    }
                    else
                    {
                        ErrorText = "Exception L402: " + System.Environment.NewLine + "Please scan a valid job tag";
                        RunOnUiThread(ShowError);
                    }                
                }
                else
            ...
            {
                ErrorText = "Exception L403: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void checkZectJob()
        {
            try
            {
                string jobRunning = Client.CheckZectJobRunning(txtItem.Text + "|" + GlobalVar.ZectWhse);
                if (jobRunning != string.Empty)
                {
                    switch (jobRunning.Split('*')[0])
                    {
                        case "1":
                            GlobalVar.RecZectJob = txtItem.Text;
                            StartActivity(typeof(ScanZectItem));
                            Finish();
                            break;
                        case "0":
                            ErrorText = "Exception L404: " + System.Environment.NewLine + jobRunning.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception L405: " + System.Environment.NewLine + jobRunning.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        default:
                            ErrorText = "Exception L406: " + System.Environment.NewLine + "Unexpected error while storing data";
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
                ...
                {
                    ErrorText = "Exception L407: " + System.Environment.NewLine + "Unexpected error while storing data";
                    RunOnUiThread(ShowError);
                }
            }
            catch (Exception ex)
            {
                ErrorText = "Exception L408: " + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
