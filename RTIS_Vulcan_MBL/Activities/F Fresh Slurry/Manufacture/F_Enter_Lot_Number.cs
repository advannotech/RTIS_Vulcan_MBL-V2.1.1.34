...
{
    [Activity(Label = "F_Enter_Lot_Number")]
    public class F_Enter_Lot_Number : Activity, View.IOnTouchListener
    {
        ...
        public void layoutStichUp()
        {
            ...
            {
                ErrorText = "Exception F101: " + System.Environment.NewLine + ex.ToString();
        ...
        private void TxtLot_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                ...
                {
                    GlobalVar.FSLotNum = txtLot.Text + "F";
                    ...
                    Thread t = new Thread(checkLotExists);
                    t.Start();
                }
                else
                {
            ...
            {
                ErrorText = "Exception F102: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }

        private void checkLotExists()
        {
            try
            {
                string lotFound = Client.CheckFSLotExists(GlobalVar.FSLotNum);
                switch (lotFound.Split('*')[0])
                {
                    case "1":
                        RunOnUiThread(showNext);
                        break;
                    case "0":
                        ErrorText = "Exception F205: " + System.Environment.NewLine + lotFound.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception F206: " + System.Environment.NewLine + lotFound.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
        ...
        private void BtnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception F103: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
