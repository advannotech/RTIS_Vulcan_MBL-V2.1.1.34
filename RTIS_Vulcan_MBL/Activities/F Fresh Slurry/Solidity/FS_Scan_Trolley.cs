...
{
    [Activity(Label = "FS_Scan_Trolley")]
    public class FS_Scan_Trolley : Activity, View.IOnTouchListener
    {
        TextView lblHeader1;
        TextView lblHeader2;
        EditText txtTrolley;
        ...
        string trolleyCode = string.Empty;
        ...
        private void TxtTrolley_KeyPress(object sender, View.KeyEventArgs e)
        {
                ...
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    if (txtTrolley.Text != string.Empty)
                    {
                        if (txtTrolley.Text.Substring(0, 4) == "TRO_" && txtTrolley.Text.Contains("$"))
                        {
                            ...
                            GlobalVar.pgsBusy.SetMessage("Checking Trolley...");
                            GlobalVar.pgsBusy.SetCancelable(false);
                            GlobalVar.pgsBusy.Show();
                            Thread t = new Thread(validateSlurry);
                            t.Start();
                        }
                        else
                        {
            ...
            {
                ErrorText = "Exception FS001: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void validateSlurry()
        {
            try
            {
                string _checked = Client.checkFreshSlurryManufactured(trolleyCode + "|" + itemCode);
                switch (_checked.Split('*')[0])
                {
                    case "1":
                        _checked = _checked.Remove(0, 2);
                        GlobalVar.FSSItemDesc = _checked.Split('|')[1];

                        GlobalVar.FSSTrolleyCode = trolleyCode;
                        GlobalVar.FSSItemCode = itemCode;
                        GlobalVar.FSSLotNum = _checked.Split('|')[0];
                        RunOnUiThread(showNext);
                        break;
                    case "0":
                        ErrorText = "Exception FS002: " + System.Environment.NewLine + _checked.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    case "-1":
                        ErrorText = "Exception FS003: " + System.Environment.NewLine + _checked.Split('*')[1];
                        RunOnUiThread(ShowError);
                        break;
                    default:
                        ErrorText = "Exception FS004: " + System.Environment.NewLine + "Unexpected error while validating item";
                        RunOnUiThread(ShowError);
                        break;
                }
            }
            ...
            {
                ErrorText = "Exception FS005: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void showNext()
        {
            ...
            {
                ErrorText = "Exception FS006: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void BtnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception FS007: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
