...
{
    [Activity(Label = "SelectType_PPRec")]

    public class SelectType_PPRec : Activity, View.IOnTouchListener
    {
        Button btnScanItem;
        ...
        {
            btnSelectItem = FindViewById<Button>(Resource.Id.btnSelect);
            btnSelectItem.SetOnTouchListener(this);
            btnSelectItem.Click += BtnSelectItem_Click;
            btnSelectItem.Visibility = ViewStates.Gone;

            btnScanItem = FindViewById<Button>(Resource.Id.btnScan);
            btnScanItem.SetOnTouchListener(this);
            btnScanItem.Click += BtnScanItem_Click;
        ...
        private void BtnSelectItem_Click(object sender, EventArgs e)
        {
            try
            {
                string whseItems = Client.GetAllWhseItems(GlobalVar.RecTransWarehouseFrom);
                if (whseItems != string.Empty)
                {
                    switch (whseItems.Split('*')[0])
                    {
                        case "1":
                            GlobalVar.RecTransType = "Select";
                            whseItems = whseItems.Remove(0, 2);
                            GlobalVar.allWIPLines = whseItems;
                            StartActivity(typeof(SelectItem_PPRec));
                            Finish();
                            break;
                        case "-1":
                            ErrorText = "Exception L001: " + System.Environment.NewLine + whseItems.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        default:
                            ErrorText = "Exception L002: " + System.Environment.NewLine + "Cannot connect to server!";
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
                ...
                {
                    ErrorText = "Exception L003: " + System.Environment.NewLine + "Cannot connect to server!";
                    RunOnUiThread(ShowError);
                }
            }
            catch (Exception ex)
            {
                ErrorText = "Exception L004: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void BtnScanItem_Click(object sender, EventArgs e)
        {
            try
            {
                GlobalVar.RecTransType = "RT2D";
                StartActivity(typeof(ScanItem_RecTrans));
                Finish();
            }
            catch (Exception ex)
            {
                ErrorText = "Exception L005: " + System.Environment.NewLine + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
