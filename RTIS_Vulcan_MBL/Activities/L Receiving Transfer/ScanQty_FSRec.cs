...
{
    [Activity(Label = "ScanQty_FSRec")]
    public class ScanQty_FSRec : Activity, View.IOnTouchListener
    {
        ...
        public void layoutStichUp()
        {
            try
            {
                ...
                lblHeader1.Text = "From WIP to Powder Prep";

                lblHeader2 = FindViewById<TextView>(Resource.Id.lblHeader2);
                lblHeader2.Text = "Scan Quantity Barcode";
        ...
        private void TxtItem_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keycode.Enter && e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        double dQty = Convert.ToDouble(txtItem.Text.Replace(",", GlobalVar.sep).Replace(".", GlobalVar.sep)) / 10000;
                        GlobalVar.RecTransQty = Convert.ToString(dQty);
                        ScanEngineOff();
                        AlertDialog.Builder build = new AlertDialog.Builder(this);
                        AlertDialog Confirm = build.Create();
                        Confirm.SetTitle("Confirm Transfer Info");
                        Confirm.SetIcon(Resource.Drawable.Icon);
                        Confirm.SetMessage(GlobalVar.RecTransItemDesc + System.Environment.NewLine + "Lot: " + GlobalVar.RecTransLotNumber + System.Environment.NewLine + "Dry Weight: " + GlobalVar.RecTransQty + System.Environment.NewLine + "Warehouse From : " + GlobalVar.RecTransWarehouseFrom + System.Environment.NewLine + "Warehouse To : " + GlobalVar.FStMSWhse);
            ...
            {
                ErrorText = "Exception L202: " + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        public void transferItem()
        {
            try
            {   //tansferFromITFSBCD
                //string transferred = Client.tansferFromTransitFS(GlobalVar.RecTransItemCode + "|" + GlobalVar.RecTransLotNumber + "|" + GlobalVar.RecTransWarehouseFrom + "|" + GlobalVar.RecTransWarehouseTo + "|" + GlobalVar.RecTransQty + "|" + GlobalVar.UserName);
                string transferred = Client.tansferFromITFSBCD(GlobalVar.RecTransItemCode + "|" + GlobalVar.RecTransLotNumber + "|" + GlobalVar.RecTransWarehouseFrom + "|" + GlobalVar.RecTransWarehouseTo + "|" + GlobalVar.RecTransQty + "|" + GlobalVar.UserName + "|" + GlobalVar.ProcessName);
                if (transferred != string.Empty)
                {
                    switch (transferred.Split('*')[0])
                    {
                        case "1":
                            GlobalVar.RecTransScannedFS.Add(GlobalVar.RccTransFSBarcode);
                            SuccessText = "Infromation Captured Successfully!";
                            RunOnUiThread(showSuccess);
                            break;
                        case "0":
                            ErrorText = "Exception L203: " + System.Environment.NewLine + transferred.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        case "-1":
                            ErrorText = "Exception L204: " + System.Environment.NewLine + transferred.Split('*')[1];
                            RunOnUiThread(ShowError);
                            break;
                        default:
                            ErrorText = "Exception L205: " + System.Environment.NewLine + "Unexpected error while transferring powder!";
                            RunOnUiThread(ShowError);
                            break;
                    }
                }
                ...
                {
                    ErrorText = "Exception L206: " + "Unexpected error while saving transfer";
                    RunOnUiThread(ShowError);
                }
            }
            catch (Exception ex)
            {
                ErrorText = "Exception L207: " + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
        private void BtnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorText = "Exception L208: " + ex.ToString();
                RunOnUiThread(ShowError);
            }
        }
